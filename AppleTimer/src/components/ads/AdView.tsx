...
export const AdView = React.memo<AdViewProps>(props => {
  ...
  const onNativeAdLoaded = (event: any) => {
    Logger('AD', 'RECIEVED', `Unified ad  Recieved, ${JSON.stringify(event)}`)
    setLoading(false)
    setLoaded(true)
    setError(false)
  ...
  const onViewableItemsChanged = (event: { viewableItems: Array<ViewToken>; changed: Array<ViewToken> }) => {
    /**
  ...
  useEffect(() => {
    ...
    return () => {
      if (!loadOnMount) {
        DeviceEventEmitter.removeListener(Events.onViewableItemsChanged, onViewableItemsChanged)
      }
    }
